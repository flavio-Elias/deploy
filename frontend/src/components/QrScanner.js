...
export default function QrScanner({ zoneId = 1, logType = 'Entrada', onSuccess }) {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', marginTop: '24px' }, children: [_jsxs("h2", { children: [t('qrScanner.title'), " ", zoneId ? `— ${t('qrScanner.zone')} ${zoneId}` : ''] }), _jsx("div", { id: containerId, style: {
                    width: 'min(320px, 100%)',
                    display: status === 'scanning' ? 'block' : 'none',
                } }), status === 'idle' && (_jsx("button", { onClick: startScanner, style: btnStyle('#1976d2'), children: t('qrScanner.startScan') })), status === 'scanning' && (_jsx("button", { onClick: reset, style: btnStyle('#757575'), children: t('qrScanner.cancel') })), (status === 'success' || status === 'error') && (_jsxs(_Fragment, { children: [_jsx("div", { style: {
                            padding: '16px 24px',
                            borderRadius: '8px',
                            ...
                            fontWeight: 600,
                            fontSize: '18px',
                        }, children: message }), _jsx("button", { onClick: reset, style: btnStyle('#1976d2'), children: t('qrScanner.scanAnother') })] }))] }));
}
